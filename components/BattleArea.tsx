...
function BattleArea(props: {
  ...
  function setStats() {
    if (player1Data) {
      const HP =
        Math.floor(
          0.01 * (2 * player1Data?.hp + 0 + Math.floor(0.25 * 1)) * level //from pokemon.com
        ) +
        level +
        10;
      setPokemonHealth(HP);
  ...
  async function handlePokemonMove(name: string) {
    //check whos turn it is then take damage accordingly also if there hp is zero
    if (isPlayer1Turn) {
      ...
      setAICurrentHP((prev) => {
        ...
        if (damage == 0) {
          setActionText(`${player1Data?.name}'s attack missed!`);
        }
        if (prev - damage <= 0) {
      ...
      setPlayer1CurrentHP((prev) => {
        ...
        if (damage == 0) {
          setActionText(`${aiData?.name}'s attack missed!`);
        }
        if (prev - damage <= 0) {
    ...
    <div className="fixed top-0 right-0 w-full h-full arena bg-gray-900">
      ...
      <div>
        <img
          ...
          } ${isAnimating && !isPlayer1Turn ? "poke-move-forward" : ""}`}
